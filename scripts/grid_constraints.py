...
def p_max_pu_EU_renewable_el(Elspotprices, CO2_emiss_El):
    """ function that enables power from the grid to be used for H2 production according to EU rules:
    1) price below limit, 2) emissionintensity below limit"""

    idx_renw_el_p = Elspotprices[Elspotprices.values <= rfnbos_dict['price_threshold']].index
    idx_renw_el_em = CO2_emiss_El[CO2_emiss_El.values <= rfnbos_dict['emission_threshold']].index
    p_max_pu_renew_el_price = pd.DataFrame(data=0, index=hours_in_period, columns=['p_max_pu el price'])
    p_max_pu_renew_em = pd.DataFrame(data=0, index=hours_in_period, columns=['p_max_pu emiss limit'])
